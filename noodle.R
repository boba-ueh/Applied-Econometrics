...
# HETEROSKEDASTICITY   
    plot(predict(model), resid(model), ylab = "Residuals", xlab = "Predicted monthly")    
    
    data <- read.csv("https://kinhteluong.online/esdata/noodle/noodle18.csv");formula <- log(noodle) ~ log(income) + price + log(distance) + gender + family + ocities + rural; model <- lm(formula, data = data);specific_tests <- c("rural=0", "ocities=0"); predictor_formula <- as.formula(paste("~", deparse(formula[[3]])));lmtest::bptest(model, predictor_formula, data = data);round(robust <- lmtest::coeftest(model, vcov = sandwich::vcovHC(model, type = "HC0")),3);car::linearHypothesis(model, paste(names(coef(model))[-1], "= 0"), white.adjust = "hc1");car::linearHypothesis(model, specific_tests, white.adjust = "hc1")
  ...
  #bai tap 8
    data <- read.csv("https://kinhteluong.online/esdata/noodle/noodle18.csv");formula <- log(noodle) ~ log(income) + price + log(distance) + gender + family + ocities + rural; model <- lm(formula, data = data);specific_tests <- list(c("rural=0", "ocities=0"), c("gender=0", "family=0"), c("log(income) = 0", "log(distance) = 0")); predictor_formula <- as.formula(paste("~", deparse(formula[[3]])));lmtest::bptest(model, predictor_formula, data = data);round(robust <- lmtest::coeftest(model, vcov = sandwich::vcovHC(model, type = "HC0")),3);cat("\n\tExact Effects Log_LogLin\n");round((1.01^coef(robust)[grepl("log", names(coef(robust)))] - 1) * 100, 3);round((exp(coef(robust)[!grepl("log", names(coef(robust))) & names(coef(robust)) != "(Intercept)"]) - 1) * 100, 3);car::linearHypothesis(model, paste(names(coef(model))[-1], "= 0"), white.adjust = "hc1");for (test_set in specific_tests) {print(car::linearHypothesis(model, test_set, white.adjust = "hc1"))}
